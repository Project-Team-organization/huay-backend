...
// หวยลาว Extra ทุกวัน เวลา 08:30 น.
...
cron.schedule('30 10 * * *', async () => {
  console.log('Fetching Lao TV lottery data...');
  try {
    await huaylaotv();
  } catch (error) {
...
cron.schedule('45 13 * * *', async () => {
  console.log('Fetching Lao HD lottery data...');
  try {
    await huaylaohd();
  } catch (error) {
...
cron.schedule('0 20 * * *', async () => {
  console.log('Fetching Lao Thakhek VIP lottery data...');
  try {
...
cron.schedule('30 21 * * *', async () => {
  console.log('Fetching Lao VIP lottery data...');
  try {
    await huaylaovip();
  } catch (error) {
...
cron.schedule('45 21 * * *', async () => {
  console.log('Fetching Lao Thakhek 5D lottery data...');
  try {
...
cron.schedule('0 22 * * *', async () => {
  console.log('Fetching Lao Stars VIP lottery data...');
  try {
...
cron.schedule('30 23 * * *', async () => {
  console.log('Fetching Lao Redcross lottery data...');
  try {
    await huylaogachad();
  } catch (error) {
