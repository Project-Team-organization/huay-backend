...
const fetchAndSaveHanoiAseanLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    const existingLottery = await LotteryHanoiAsean.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอยอาเซียน วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยฮานอยอาเซียน วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-asean/latest"
    );
    ...
    if (!data.results.prize_1st) {
      throw new Error(
        `Failed to fetch and save Hanoi ASEAN lottery: หวยฮานอยอาเซียน วันนี้ยังไม่ออกผล`
      );
    }
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiAsean.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
      console.log(`🔄 อัพเดทข้อมูลหวยฮานอยอาเซียน วันนี้`);
    } else {
      lottery = new LotteryHanoiAsean(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยฮานอยอาเซียน วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Hanoi ASEAN lottery: ${error.message}`);
  }
};

const getAllHanoiAseanLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiAsean.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiAsean.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiAseanLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยอาเซียน");
  }
};

const getHanoiAseanLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiAsean.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiAseanLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยอาเซียน");
  }
};

const createHanoiAseanLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiAsean(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiAseanLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอยอาเซียน");
  }
};

const updateHanoiAseanLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiAsean.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiAseanLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอยอาเซียน");
  }
};

const deleteHanoiAseanLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiAsean.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiAseanLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอยอาเซียน");
  }
};

module.exports = {
  fetchAndSaveHanoiAseanLottery,
