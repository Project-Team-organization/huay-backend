...
exports.createBettingType = async (data) => {
  try {
    const exists = await bettingTypes.findOne({ lottery_type: data.lottery_type });
    if (exists) {
      throw new Error('Lottery type นี้มีอยู่แล้ว');
    }

    const newBettingType = await bettingTypes.create(data);
