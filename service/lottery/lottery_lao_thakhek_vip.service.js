...
const apiUrl = 'https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/lao-thakhek-vip/latest';
...
const fetchAndSaveLaoThakhekVipLottery = async () => {
  try {
    ...
    if (!data.results || !data.results.digit4 || data.results.digit4 === "xxx") {
      throw new Error(
        `Failed to fetch and save Lao Thakhek VIP lottery: หวยลาวท่าแขก VIP วันนี้ยังไม่ออกผล`
      );
    }
...
const getAllLaoThakhekVipLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryLaoThakhekVip.countDocuments(query);

    // Get data with pagination
    const data = await LotteryLaoThakhekVip.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllLaoThakhekVipLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยลาวท่าแขก VIP");
  }
};

module.exports = {
  fetchAndSaveLaoThakhekVipLottery,
