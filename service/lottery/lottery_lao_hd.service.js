...
const apiUrl = 'https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/lao-hd/latest';
...
const fetchAndSaveLaoHdLottery = async () => {
  try {
    ...
    if (data.results.digit3 && data.results.digit3.length === 3) {
      oneTop = data.results.digit3.split("").join(","); // เช่น "686" → "6,8,6"
    }

    // ➤ 1 ตัวล่าง (วิ่งล่าง จาก digit2_bottom)
    let oneBottom = "";
    if (data.results.digit2_bottom && data.results.digit2_bottom.length === 2) {
      oneBottom = data.results.digit2_bottom.split("").join(","); // เช่น "97" → "9,7"
    }
...
const getAllLaoHdLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryLaoHd.countDocuments(query);

    // Get data with pagination
    const data = await LotteryLaoHd.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllLaoHdLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยลาว HD");
  }
};

module.exports = {
  fetchAndSaveLaoHdLottery,
  getAllLaoHdLottery,
};
