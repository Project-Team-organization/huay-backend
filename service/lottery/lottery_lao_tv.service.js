  ...
  try {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยลาว TV วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยลาว TV วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryLaoTv.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
      console.log(`🔄 อัพเดทข้อมูลหวยลาว TV วันนี้`);
    } else {
      lottery = new LotteryLaoTv(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยลาว TV วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
