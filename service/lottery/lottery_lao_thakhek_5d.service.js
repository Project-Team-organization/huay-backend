...
const apiUrl = 'https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/lao-thakhek-5d/latest';

const fetchAndSaveLaoThakhek5dLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    // ถ้าเวลาเป็น 00:00:00 ก็ต้องหาในวันก่อนหน้า
    const existingLottery = await LotteryLaoThakhek5d.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยลาวท่าแขก 5D วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยลาวท่าแขก 5D วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }
    ...
    if (!data.results || !data.results.digit4 || data.results.digit4 === "xxx") {
      throw new Error(
        `Failed to fetch and save Lao Thakhek 5D lottery: หวยลาวท่าแขก 5D วันนี้ยังไม่ออกผล`
      );
    }
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryLaoThakhek5d.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
      console.log(`🔄 อัพเดทข้อมูลหวยลาวท่าแขก 5D วันนี้`);
    } else {
      lottery = new LotteryLaoThakhek5d(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยลาวท่าแขก 5D วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Lao Thakhek 5D lottery: ${error.message}`);
  }
};

const getAllLaoThakhek5dLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryLaoThakhek5d.countDocuments(query);

    // Get data with pagination
    const data = await LotteryLaoThakhek5d.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllLaoThakhek5dLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยลาวท่าแขก 5D");
  }
};

module.exports = {
  fetchAndSaveLaoThakhek5dLottery,
