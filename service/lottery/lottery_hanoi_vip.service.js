...
const fetchAndSaveHanoiVipLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    const existingLottery = await LotteryHanoiVip.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอย VIP วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      console.log(`⏳ หวยฮานอย VIP วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-vip/latest"
    );
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiVip.findByIdAndUpdate(existingLottery._id, lotteryData, { new: true });
      console.log(`🔄 อัพเดทข้อมูลหวยฮานอย VIP วันนี้`);
    } else {
      lottery = new LotteryHanoiVip(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยฮานอย VIP วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Hanoi VIP lottery: ${error.message}`);
  }
};

const getAllHanoiVipLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiVip.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiVip.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiVipLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอย VIP");
  }
};

const getHanoiVipLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiVip.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiVipLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอย VIP");
  }
};

const createHanoiVipLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiVip(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiVipLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอย VIP");
  }
};

const updateHanoiVipLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiVip.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiVipLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอย VIP");
  }
};

const deleteHanoiVipLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiVip.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiVipLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอย VIP");
  }
};

module.exports = {
  fetchAndSaveHanoiVipLottery,
