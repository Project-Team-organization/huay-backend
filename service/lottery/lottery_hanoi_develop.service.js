...
const fetchAndSaveHanoiDevelopLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    const existingLottery = await LotteryHanoiDevelop.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอยพัฒนา วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      console.log(`⏳ หวยฮานอยพัฒนา วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-develop/latest"
    );
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiDevelop.findByIdAndUpdate(existingLottery._id, lotteryData, { new: true });
      console.log(`🔄 อัพเดทข้อมูลหวยฮานอยพัฒนาวันนี้`);
    } else {
      lottery = new LotteryHanoiDevelop(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยฮานอยพัฒนาวันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Hanoi Develop lottery: ${error.message}`);
  }
};

const getAllHanoiDevelopLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiDevelop.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiDevelop.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiDevelopLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยพัฒนา");
  }
};

const getHanoiDevelopLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiDevelop.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiDevelopLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยพัฒนา");
  }
};

const createHanoiDevelopLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiDevelop(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiDevelopLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอยพัฒนา");
  }
};

const updateHanoiDevelopLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiDevelop.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiDevelopLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอยพัฒนา");
  }
};

const deleteHanoiDevelopLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiDevelop.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiDevelopLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอยพัฒนา");
  }
};

module.exports = {
  fetchAndSaveHanoiDevelopLottery,
