...
const fetchAndSaveHanoiSpecialLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    const existingLottery = await LotteryHanoiSpecial.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอยเฉพาะกิจ วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยฮานอยเฉพาะกิจ วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-special/latest"
    );
    ...
    if (!data.results.prize_1st) {
      throw new Error(
        `Failed to fetch and save Hanoi Special lottery: หวยฮานอยเฉพาะกิจ วันนี้ยังไม่ออกผล`
      );
    }
    ...
    const lotteryData = {
      name: data.name,
      url: data.url,
      title: data.title || "ฮานอยเฉพาะกิจ",
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiSpecial.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
      console.log(`🔄 อัพเดทข้อมูลหวยฮานอยเฉพาะกิจ วันนี้`);
    } else {
      lottery = new LotteryHanoiSpecial(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยฮานอยเฉพาะกิจ วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Hanoi Special lottery: ${error.message}`);
  }
};

const getAllHanoiSpecialLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiSpecial.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiSpecial.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiSpecialLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยเฉพาะกิจ");
  }
};

const getHanoiSpecialLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiSpecial.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiSpecialLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยเฉพาะกิจ");
  }
};

const createHanoiSpecialLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiSpecial(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiSpecialLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอยเฉพาะกิจ");
  }
};

const updateHanoiSpecialLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiSpecial.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiSpecialLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอยเฉพาะกิจ");
  }
};

const deleteHanoiSpecialLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiSpecial.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiSpecialLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอยเฉพาะกิจ");
  }
};

module.exports = {
  fetchAndSaveHanoiSpecialLottery,
