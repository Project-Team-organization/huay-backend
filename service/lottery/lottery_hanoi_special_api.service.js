...
const fetchAndSaveHanoiSpecialApiLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    const existingLottery = await LotteryHanoiSpecialApi.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอยพิเศษ วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยฮานอยพิเศษ วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-special-api/latest"
    );
    ...
    if (!data.results.prize_1st) {
      throw new Error(
        `Failed to fetch and save Hanoi Special API lottery: หวยฮานอยพิเศษ วันนี้ยังไม่ออกผล`
      );
    }
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiSpecialApi.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
      console.log(`🔄 อัพเดทข้อมูลหวยฮานอยพิเศษ วันนี้`);
    } else {
      lottery = new LotteryHanoiSpecialApi(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยฮานอยพิเศษ วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Hanoi Special API lottery: ${error.message}`);
  }
};

const getAllHanoiSpecialApiLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiSpecialApi.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiSpecialApi.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiSpecialApiLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยพิเศษ");
  }
};

const getHanoiSpecialApiLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiSpecialApi.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiSpecialApiLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยพิเศษ");
  }
};

const createHanoiSpecialApiLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiSpecialApi(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiSpecialApiLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอยพิเศษ");
  }
};

const updateHanoiSpecialApiLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiSpecialApi.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiSpecialApiLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอยพิเศษ");
  }
};

const deleteHanoiSpecialApiLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiSpecialApi.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiSpecialApiLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอยพิเศษ");
  }
};

module.exports = {
  fetchAndSaveHanoiSpecialApiLottery,
