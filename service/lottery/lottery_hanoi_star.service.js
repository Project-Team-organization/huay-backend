...
const fetchAndSaveHanoiStarLottery = async () => {
  try {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอยสตาร์ วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยฮานอยสตาร์ วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-star/latest"
    );
    ...
    if (!data.results.prize_1st) {
      throw new Error(
        `Failed to fetch and save Hanoi Star lottery: หวยฮานอยสตาร์ วันนี้ยังไม่ออกผล`
      );
    }
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiStar.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
    ...
    throw new Error(`Failed to fetch and save Hanoi Star lottery: ${error.message}`);
  }
};

const getAllHanoiStarLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiStar.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiStar.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiStarLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยสตาร์");
  }
};

const getHanoiStarLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiStar.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiStarLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยสตาร์");
  }
};

const createHanoiStarLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiStar(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiStarLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอยสตาร์");
  }
};

const updateHanoiStarLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiStar.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiStarLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอยสตาร์");
  }
};

const deleteHanoiStarLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiStar.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiStarLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอยสตาร์");
  }
};

module.exports = {
  fetchAndSaveHanoiStarLottery,
