...
const fetchAndSaveHanoiRedcrossLottery = async () => {
  try {
    // เช็คถ้าวันนี้มีข้อมูลแล้ว และผลหวยออกครบแล้ว ไม่ต้องอัพอีก
    const today = new Date();
    const existingLottery = await LotteryHanoiRedcross.findOne({
      createdAt: {
    ...
    if (existingLottery && existingLottery.results) {
      ...
      if (!hasIncompleteResults) {
        console.log(`✅ หวยฮานอยกาชาด วันนี้มีข้อมูลครบแล้ว ไม่ต้องอัพเดท`);
        return existingLottery;
      }
      
      console.log(`⏳ หวยฮานอยกาชาด วันนี้มีข้อมูลแต่ยังไม่ออกครบ จะอัพเดทใหม่`);
    }

    const response = await axios.get(
      "https://test-lotto-scraper.wnimqo.easypanel.host/api/lottery/hanoi-redcross/latest"
    );
    ...
    if (!data.results.prize_1st) {
      throw new Error(
        `Failed to fetch and save Hanoi Redcross lottery: หวยฮานอยกาชาด วันนี้ยังไม่ออกผล`
      );
    }
    ...
    let lottery;
    if (existingLottery) {
      lottery = await LotteryHanoiRedcross.findByIdAndUpdate(
        existingLottery._id,
        lotteryData,
        { new: true }
      );
      console.log(`🔄 อัพเดทข้อมูลหวยฮานอยกาชาด วันนี้`);
    } else {
      lottery = new LotteryHanoiRedcross(lotteryData);
      await lottery.save();
      console.log(`💾 บันทึกข้อมูลหวยฮานอยกาชาด วันนี้ใหม่`);
    }
    return lottery;
  } catch (error) {
    throw new Error(`Failed to fetch and save Hanoi Redcross lottery: ${error.message}`);
  }
};

const getAllHanoiRedcrossLottery = async ({ page, limit, startDate, endDate }) => {
  try {
    const query = {};
    ...
    const total = await LotteryHanoiRedcross.countDocuments(query);

    // Get data with pagination
    const data = await LotteryHanoiRedcross.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    ...
    console.error("Error in getAllHanoiRedcrossLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยกาชาด");
  }
};

const getHanoiRedcrossLotteryById = async (id) => {
  try {
    const lottery = await LotteryHanoiRedcross.findById(id);
    return lottery;
  } catch (error) {
    console.error("Error in getHanoiRedcrossLotteryById service:", error);
    throw new Error("เกิดข้อผิดพลาดในการดึงข้อมูลหวยฮานอยกาชาด");
  }
};

const createHanoiRedcrossLottery = async (lotteryData) => {
  try {
    const lottery = new LotteryHanoiRedcross(lotteryData);
    await lottery.save();
    return lottery;
  } catch (error) {
    console.error("Error in createHanoiRedcrossLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการสร้างข้อมูลหวยฮานอยกาชาด");
  }
};

const updateHanoiRedcrossLottery = async (id, updateData) => {
  try {
    const lottery = await LotteryHanoiRedcross.findByIdAndUpdate(id, updateData, { new: true });
    return lottery;
  } catch (error) {
    console.error("Error in updateHanoiRedcrossLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการอัพเดทข้อมูลหวยฮานอยกาชาด");
  }
};

const deleteHanoiRedcrossLottery = async (id) => {
  try {
    const lottery = await LotteryHanoiRedcross.findByIdAndDelete(id);
    return lottery;
  } catch (error) {
    console.error("Error in deleteHanoiRedcrossLottery service:", error);
    throw new Error("เกิดข้อผิดพลาดในการลบข้อมูลหวยฮานอยกาชาด");
  }
};

module.exports = {
  fetchAndSaveHanoiRedcrossLottery,
