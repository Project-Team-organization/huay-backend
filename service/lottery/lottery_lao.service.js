...
const fetchAndSaveLaoLottery = async () => {
  try {
    ...
    const existingLottery = await LotteryLao.findOne({ createdAt: { $gte: new Date(today.setHours(0, 0, 0, 0)) , $lt: new Date(today.setHours(23, 59, 59, 999)) } });
    ...
    if(data.numbers.tail4 == 'xxxx'){
       throw new Error(`Failed to fetch and save Lao lottery: หวยลาววันนี้ยังไม่ออกผล`);
    }
